...
# BoringSSL checkout.
#
# On Android the break tests are performed on libcrypto.so as there is no support
# for static binaries with FIPS mode enabled.
...
usage() {
  echo "USAGE: $0 [local|device] [32|64]"
  echo "local:  Assume local BoringSSL build"
  echo "device: Assume Android build for testing on attached device"
  echo "32/64:  Android build variant"
  if [ "$1" ]; then
    echo
    echo "$1"
  fi
...
test "$MODE" || usage "Unable to infer mode, please specify on the command line."
...
if [ "$MODE" = "local" ]; then
  ...
  test "$TARGET_PRODUCT" = "aosp_arm64" || die "Lunch target is $TARGET_PRODUCT but expected aosp_arm64"
...
RUN="${MODE}_run"
INTEGRITY_BREAK_TEST="${MODE}_integrity_break_test"
KAT_BREAK_TEST="${MODE}_kat_break_test"
RUNTIME_BREAK_TEST="${MODE}_runtime_break_test"
