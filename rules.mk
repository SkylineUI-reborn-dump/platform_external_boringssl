...
# ghash and chacha assembly files use neon instructions, which we aren't allowed
# to do in the kernel if ALLOW_FP_USE is disabled. This means that the kernel
# can't use ghash or chacha, but we don't need to for now. If someone ever tries
# to, we will get missing symbols during linking.
LOCAL_SRC_FILES_$(ARCH) := $(filter-out linux-aarch64/crypto/fipsmodule/ghash-neon-armv8.S,$(LOCAL_SRC_FILES))
LOCAL_SRC_FILES_$(ARCH) := $(filter-out linux-aarch64/crypto/chacha/chacha-armv8.S,$(LOCAL_SRC_FILES))
endif
